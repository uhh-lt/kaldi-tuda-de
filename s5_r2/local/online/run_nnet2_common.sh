...
for line in sys.stdin.readlines():
    if len(line.strip()) == 0:
        continue
    split = line.split()
    print split[0], split[1] #'sox {0} --vol {1} -t wav - -t wav - |'.format(split[1].strip(), random.uniform(scale_low, scale_high))
...
if [ $stage -le 2 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
    ...
    exp/tri3b_ali exp/nnet2_online/tri4
fi


if [ $stage -le 3 ]; then
  mkdir -p exp/nnet2_online
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 5 --num-frames 700000 \
