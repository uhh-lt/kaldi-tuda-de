...
if [ $stage -le 9 ]; then
  ...
  for decode_set in dev test; do
      ...
      steps/nnet2/decode.sh --nj $num_jobs --cmd "$decode_cmd" --config conf/decode.config \
        ...
        exp/tri3b/graph data/${decode_set}_hires $dir/decode_${decode_set} || exit 1;
  done
fi
...
if [ $stage -le 11 ]; then
  ...
  for decode_set in dev test; do
    ...
    steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $num_jobs \
      exp/tri3b/graph data/${decode_set}_hires ${dir}_online/decode_${decode_set} || exit 1;
  done
fi
...
if [ $stage -le 12 ]; then
  ...
  for decode_set in dev test; do
      num_jobs=`cat data/${decode_set}_hires/utt2spk|cut -d' ' -f2|sort -u|wc -l`
      steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $num_jobs \
        --per-utt true exp/tri3b/graph data/${decode_set}_hires ${dir}_online/decode_${decode_set}_utt || exit 1;
  done
fi
...
if [ $stage -le 13 ]; then
  ...
  for decode_set in dev test; do
      num_jobs=`cat data/${decode_set}_hires/utt2spk|cut -d' ' -f2|sort -u|wc -l`
      steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $num_jobs \
        --per-utt true --online false exp/tri3b/graph data/${decode_set}_hires \
